...
#load packages
packages <- c("coin", 'ggbeeswarm', 'lsr','cowplot', 'BayesFactor', 'readr', 'plyr', 'jsonlite', 'ggplot2', 'scales', "ggExtra", 'coefplot', "grid", 'corrplot', 'ggsignif')
...
```{r}
gpDF <- ddply(subset(paramEstimates, kernel=='RBF') , ~participant+environment+context+meanScore+R2, plyr::summarize, lambda = mean(lambda), beta = mean(beta), tau = mean(tau))
library('GGally')
  
ggpairs(gpDF[,c('tau', 'R2', 'meanScore', 'context')], mapping = aes(color = context))
```
